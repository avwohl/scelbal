...
0.1	AUG 1988	Fixed a bug in the command line parser that puts it
	...
	Feb 2014	changed to 8008 assembler, default radix octal, # as numeric separator

	Oct 2019	changed "*" to "$" as program counter value, lost $nnn as result
	...
	switch (token.attr & TYPE) {
	    ...
	    case STR:	for (;;) {
				...
				case OPR:   if (!(token.attr & BINARY)) {
						...
						case '#':   u = (u *256) + v; break; //HRJ split octal
...
TOKEN *lex(void)
{
    ...
    if (isalph(c = popc())) {
		...
		pushc(c);  pops(token.sval);
		for (p = token.sval; *p; ++p); /* HRJ get last char */
		switch (toupper(*--p)) {
	    	case 'B':	b = 2;  break;

	    	default:	++p;	/* HRJ new default octal */
	    	case 'O':
	    	case 'Q':	b = 8;  break;

	    	/* default:	++p; HRJ old default decimal */
	    	case 'D':	b = 10;  break;
	...
	case '$':   b = 16; /*HRJ dosesnt get here due to strcmp $ test above */
num:		    pops(token.sval);
		    	make_number(b);
		    	break;
	...
	case '#':   token.attr = BINARY + OPR; goto opr1; //HRJ split octal?
...
unsigned base;
{
	...
	//HRJ test and fix for split-octal without delimiter like nnn#nnn
	// if value exceeds 255d then was not split, must adjust zyzzy
	if ((base == 8) && (split_val) && ((token.valu) > 255)) {
		valu_hi = (token.valu / 256) * 256; // high byte is twice high
		valu_lo = token.valu - (valu_hi);  // get low byte
		token.valu = valu_hi/2 + valu_lo;  // adjust high value
	}
    clamp(token.valu); // max value FFFFH
    return;
}
...
{
    // old HRJ return (c >= '@' && c <= '~') || (c >= '#' && c <= '&') ||
	// old HRJ c == '!' || c == '.' || c == ':' || c == '?';
	return (c >= 'A' && c <= '~') || c == '!' || c == '$' || // HRJ base conversion % @
	c == '&' || c == '.' || c == ':' || c == '?';
}
