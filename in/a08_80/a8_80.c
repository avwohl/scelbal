...
0.1+	Dec 2013	A85: edited for LCC-32 by Herb Johnson
	...
	Jan 2014	HRJ converted to A08 for 8008 assembly
	Oct 2019  HRJ asm program counter was "*", now is "$".
	              Can do @nnn, %nnn but can't do $nnn without converting to a18eval.c code
	Mar 2020 HRJ 8008 1972 mnemonics -> 8080 op codes
	Jan 2022 HRJ NOT 16 bit value not processed right, FF80 -> FF00
...
unsigned split_val; // HRJ true if split_octal, set by SPLIT pseudoop
...
char **argv;
{
    ...
    printf("8008 1972 to 8080 ops Cross-Assembler Ver 0.1+\n");
    printf("Copyright (c) 1985,1987 William C. Colley, III\n\n");
    printf("(C) Herb Johnson fixes for LCC/Windows, 8008, octal radix, etc Apr 2024 \n\n");
	...
	split_val = FALSE; //HRJ true if split octal values
	while (!done) {
	    errcode = ' ';
...
static void normal_op(void)
{
	...
	switch (attrib & ARG1) {
	    ...
	    case PORT:	    unlex(); u = expr();
			    if (obj[0]==0xDB) {  //HRJ in, must be 0-7 value
				   if ((0 <= u)  && (u <= 7)) { /* compiler complaint is OK */
				      obj[1] = low(u);  break;
				   }
				   else { error('V');  u = 0; break; }
			    }
			    else if (obj[0]==0xD3) { //HRJ out 10- 37 octal value
				   if ((8 <= u)  && (u <= 0x1F)) {
				      obj[1] = low(u);  break;
				   }
				   else { error('V');  u = 0; break; }
			    }
			    else { error('^'); u = 0; break; } //HRJ not in or out?

	    case RST_NUM:   unlex();
				/* HRJ support RST 0-70 (no) and RST 0-7 */
				u = expr();
			    //if ( (u/8 <= 7) && (u >= 8) ) {
				//	obj[0] = 0x05 + u; break;
				//}
			    if (u <= 7) {
					obj[0] = 0xC7 +(u * 8);  break;
				} else { error('V');  u = 0; break; }
...
do_reg:	    if ((token.attr & TYPE) != REG) {
			...
			if (token.valu != SP) { error('R'); break; } //HRJ LXI SP only
...
save_byte:				*o++ = low(u);  ++bytes;  break;
	...
	case SPLIT: listhex = FALSE; do_label();
			//HRJ if SPLIT is TRUE (any value), split octal values without #
			if ((lex() -> attr & TYPE) != EOL) {
				unlex(); split_val = expr();
				if (split_val > 0) split_val = TRUE;
			}
			break;
